...
public abstract class TransformTransportToFormat<T>
        extends PTransform<PCollection<EventTransport>, PCollection<T>> {
  ...
  @Override
  public PCollection<T> expand(PCollection<EventTransport> input) {
    return null;
  }
  ...
  static abstract class TransformTransport<T> extends DoFn<EventTransport, T> {
    ...
    protected abstract void setupSpecific() throws Exception;
  }
  ...
  public static class TransformToGenericRecords extends TransformTransportToFormat<GenericRecord> {
    ...
    @Override
    public PCollection<GenericRecord> expand(PCollection<EventTransport> input) {
      var options = input.getPipeline().getOptions().as(EventPayloadOptions.class);
      return input
              .apply("TransformToGenericRecord", ParDo.of(new TransformTransportToGenericRecord(
                      options.getClassName(),
                      options.getAvroSchemaLocation(),
                      options.getEventFormat())))
              .setCoder(
                      AvroGenericCoder.of(retrieveAvroSchema(
                              input.getPipeline().getOptions().as(EventPayloadOptions.class))));
    }
  ...
  public static class TransformToTableRows extends TransformTransportToFormat<TableRow> {
    ...
    @Override
    public PCollection<TableRow> expand(PCollection<EventTransport> input) {
      var options = input.getPipeline().getOptions().as(EventPayloadOptions.class);
      return input
              .apply("TransformToGenericRecord",
                      ParDo.of(new TransformTransportToTableRow(
                              options.getClassName(),
                              options.getAvroSchemaLocation(),
                              options.getEventFormat())));
    }
